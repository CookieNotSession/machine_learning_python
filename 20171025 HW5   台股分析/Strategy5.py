...
for i in range(len(tradeday)): #把分鐘資料找出來
    ...
    if(len(idx2)==0 and len(idx3)==0): #沒遇到30與-30 不出手
        ...
        p2 = TAIEX[idx[idx2[0]],1] #買進價格
        indexk = -1
        #p3 = TAIEX[idx[-1],1] #整天結束 賣出價格
        #profit.append(p3-p2) #沒遇到停損點時 的收益
        idx4 = np.nonzero(TAIEX[idx,4]<=p2-30)[0] #買進時 上漲停損點(比買進價格再低30點)
        if len(idx4)!=0:
        ...
        p2 = TAIEX[idx[idx3[0]],1] #買進價格
        indexk = -1
        #p3 = TAIEX[idx[-1],1] #整天結束 賣出價格
        #profit.append(p3-p2) #沒遇到停損點時 的收益
        idx4 = np.nonzero(TAIEX[idx,3]>=p2+30)[0] #買進時 上漲停損點(比買進價格再低30點)
        if len(idx4)!=0:
            for k in range(len(idx4)):
                if(idx4[k]>idx3[0]):
                    indexk=k
