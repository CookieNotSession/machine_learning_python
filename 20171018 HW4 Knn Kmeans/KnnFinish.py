...
for i in range(10):
    Matrix = np.zeros((3,3)) #做完一次nn 就要把矩陣清空一次
    for a in range(150): #橫掃每回合test data那朵花 距離最近的花是編號多少 
        vote = np.zeros(3,int)
        for knn in range(i+1): #用knn來直掃 看這次是幾nn 要掃幾條直的
            #Target[CheckBox[a,knn]]
            index = CheckBox[a,knn]
            vote[Target[index]]+=1 #每次nn 就在裡面相對位置投一票
