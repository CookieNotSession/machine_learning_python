...
for i in range(len(tradeday)): #把分鐘資料找出來
    ...
    if(len(idx2)==0 and len(idx3)==0): #沒遇到30與-30 不出手
        ...
        profit.append(p3-p2)
        idx4 = np.nonzero(TAIEX[idx,3]>=TAIEX[idx[idx2[0]],1])
        for j in range(len(idx4)):
            if(idx4[j]>idx[idx2[0]]):
                p4 = TAIEX[idx[idx2[0]],1]
                profit.append(p3-p4)
    elif(len(idx2)==0): #放空
        p2 = TAIEX[idx[idx3[0]],1] #買
        p3 = TAIEX[idx[-1],1] #賣
        profit.append(p2-p3)
        idx4 = np.nonzero(TAIEX[idx,4]>=TAIEX[idx[idx3[0]],1])
        for j in range(len(idx4)):
            if(idx4[j]>idx[idx3[0]]):
                p4 = TAIEX[idx[idx3[0]],1]
                profit.append(p4-p3)
    elif(idx2[0]<idx3[0]):
        p2 = TAIEX[idx[idx2[0]],1] #買
        p3 = TAIEX[idx[-1],1] #賣
